...
#'
#' @param image gsimage matrix object.
...
ConvolutePixels <- function(image, convolution.factor = 2) {
  ...
  # area.
  new.mat = matrix(nrow = nrow(image) / convolution.factor, ncol = ncol(image) / convolution.factor)
  for (r in seq(1,nrow(image), convolution.factor)) {
    for (c in seq(1,ncol(image), convolution.factor)) {
      pxl = image[r:(r+convolution.factor-1),c:(c+convolution.factor-1)]
      new.mat[r %/% convolution.factor + 1, c %/% convolution.factor + 1] = mean(pxl)
    }
  }
  class(new.mat) = 'gsimage'
  return (new.mat)
}
